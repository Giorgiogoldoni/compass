...
def run_backtest(livello_id, etf_data_completo, alloc, n_etf_max,
    ...
    for data_corrente in all_dates:
        ...
        # Registra equity curve giornaliera
    ...
    for v in versioni:
        ...
        for u in v["etf_usciti_anticipati"]:
            u.pop("_key", None)
        # Calcola metriche rischio per ogni versione
        eq = v.pop("_equity", [])
        v["volatilita_ann"] = calc_volatilita(eq)
        v["max_drawdown"] = calc_max_drawdown(eq)
        v["sharpe"] = calc_sharpe(eq)

    # Metriche sull'intera equity curve (tutte le versioni concatenate)
    ...
    # Aggiungi metriche totali al portafoglio (lette da gestisci_portafogli)
    for v in versioni:
        v["_vol_tot"] = vol_tot
        v["_mdd_tot"] = mdd_tot
...
def gestisci_portafogli(portafogli_esistenti, etf_data, regime_corrente,
    ...
    for lv in levels_data:
        ...
        # Metriche rischio totale (calcolate nel backtest)

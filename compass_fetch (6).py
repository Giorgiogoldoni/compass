...
ETF_UNIVERSE = sorted(set([
    ...
    "EXV1.DE",    "EXX5.DE",    "EXXW.DE",    "FGEQ.F",    "GOM.MI",    "HYLD.MI",
    "IBGM.MI",    "IBGS.MI",    "IBTM.MI",    "IDVY.MI",    "IEAC.MI",    "IEAG.MI",
    "IFFF.MI",    "IGLO.DE",    "IHYU.MI",    "IPRP.MI",    "IS3N.DE",    "ISPA.DE",
    "IU0E.MI",    "IUIT.L",    "IUSA.MI",    "IWDP.MI",    "JNHD.DE",    "JPNH.MI",
    "MEUD.MI",    "NTSG.MI",    "NTSX.MI",    "NTSZ.MI",    "PHAG.MI",    "PHAU.MI",
    "QNTM.MI",    "QQQ3.MI",    "RARE.MI",    "SEGA.MI",    "SEMB.DE",    "SMART.MI",
    "SMH.MI",    "STHE.MI",    "SWDA.MI",    "TDIV.MI",    "VAGE.DE",    "VAPX.MI",
    "VHYL.MI",    "VUSA.MI",    "VWCE.DE",    "WENT.MI",    "WHCS.MI",    "WRTY.MI",
    "WS5X.MI",    "WSPE.MI",    "WSPX.MI",    "WWRD.MI",    "XAIX.MI",    "XDWT.MI",
    "XEOD.DE",    "XEON.MI",    "XREA.DE",    "XUCS.DE",    "XUTC.MI",
]))
...
ETF_CATEGORIA = {
    ...
    "GOM.MI":"obbligaz_ig","AGGH.DE":"obbligaz_ig","IGLO.DE":"obbligaz_ig",
    "IEAC.MI":"obbligaz_ig","SEGA.MI":"obbligaz_ig","VAGE.DE":"obbligaz_ig",
    "EUNH.DE":"obbligaz_ig","IBGS.MI":"obbligaz_ig","XUTC.MI":"obbligaz_ig",
    ...
    "NTSG.MI":"az_globale","XREA.DE":"az_globale","XREA.DE":"az_globale",
...
ETF_TIPO = {
    ...
    "IBTM.MI":"Dist","GOM.MI":"Dist","AGGH.DE":"Acc","IGLO.DE":"Dist",
    ...
    "WRTY.MI":"Acc","VAPX.MI":"Dist","JPNH.MI":"Dist","IS3N.DE":"Acc",
...
ETF_NOMI = {
    ...
    "AGGH.DE":"iShares Core Global Aggregate Bond UCITS ETF",
    "IGLO.DE":"iShares Global Govt Bond UCITS ETF",
    ...
    "JNHD.DE":"JPMorgan EUR Corporate Bond Research Enhanced",
...
def get_price_on_date(ticker, target_date, etf_data_completo):
    ...
    dates = sig.get("dates", [])
    if not closes or not dates: return None
    # Trova la data più vicina <= target_date
    best_price = None
    for i, d in enumerate(dates):
        if d <= target_date:
            best_price = closes[i]
...
def seleziona_etf_per_data(pool, etf_data_completo, livello_id, alloc_t,
    ...
    """Seleziona gli ETF migliori per un livello in una data specifica."""
    isDist = livello_id.startswith("C")
    candidati = []
    for t in pool:
        ...
        if score <= 0: continue
        cat = ETF_CATEGORIA.get(t, "az_globale")
        # Applica peso macro se disponibile
...
def run_backtest(livello_id, etf_data_completo, alloc, n_etf_max,
    ...
    # Stato del regime simulato durante il backtest
    # Usiamo il regime corrente per tutto il periodo (semplificazione realistica:
    # il regime viene calcolato dai proxy USA che abbiamo già scaricato)
    # Per il backtest storico usiamo una progressione plausibile basata sui periodi:
    # 2025 H1: goldilocks → 2025 H2: goldilocks/neutro → 2026 Q1: risk_off (dazi) → 2026 Q2: reflazione
    def get_regime_for_date(d):
        if d < "2025-03-01": return ("goldilocks", 65)
        if d < "2025-06-01": return ("goldilocks", 60)
        if d < "2025-09-01": return ("neutro", 58)
        if d < "2025-12-01": return ("goldilocks", 62)
        if d < "2026-02-01": return ("neutro", 55)
        if d < "2026-04-15": return ("risk_off", 68)   # crollo dazi aprile
        return ("reflazione", 67)

    regime_storia_bt = []  # storia settimanale del regime
    ...
    dd_trigger_forte = -30  # tutti i livelli
    ...
    for data_corrente in all_dates:
        ...
        # Calcola giorni attivi versione corrente
        try:
        ...
        # ── Aggiorna prezzi e performance giornaliera ──────────────────
        tot_valore = 0.0
        trigger_dd = None
        ...
        for etf in versione_attiva["composizione"]:
            ...
            if dd <= -50 and not trigger_dd:
                trigger_dd = {"ticker": t, "tipo": "drawdown_50", "dd": dd,
                              "motivo": f"{t} drawdown {dd:.1f}% — uscita immediata"}
            elif dd <= dd_trigger_forte and not trigger_dd:
                trigger_dd = {"ticker": t, "tipo": "drawdown_30", "dd": dd,
                              "motivo": f"{t} drawdown {dd:.1f}% — ribilanciamento obbligatorio"}
            elif dd <= dd_trigger_medio and not trigger_dd:
                trigger_dd = {"ticker": t, "tipo": "drawdown_20", "dd": dd,
                              "motivo": f"{t} drawdown {dd:.1f}% — ribilanciamento aggressivo"}
            elif dd <= -10:
                # Alert: salva tra gli usciti anticipati se non già presente
                alert_key = f"{t}_{data_corrente}"
            ...
            valore_etf = etf["importo"] * (1 + tr / 100)
            tot_valore += valore_etf
        ...
        versione_attiva["performance_pct"] = round(perf_tot_eur / versione_attiva["capitale_inizio"] * 100, 2)
        versione_attiva["aggiornato"] = data_corrente

        # ── Check regime settimanale (ogni 5 giorni di trading) ────────
        try:
            d_open = datetime.date.fromisoformat(ultimo_check_regime)
            d_curr = datetime.date.fromisoformat(data_corrente)
            giorni_da_check = (d_curr - d_open).days
        except Exception:
            giorni_da_check = 0
        ...
        if giorni_da_check >= 5:
            ...
            if sc != regime_attivo:
                ...
                soglia_settimane = pers["settimane"]
                soglia_conf = pers["soglia"]
                if settimane_nuovo_regime >= soglia_settimane and conf >= soglia_conf:
        ...
        # ── Gestisci trigger ───────────────────────────────────────────
        deve_ribilanciare = None
        if trigger_dd:
            deve_ribilanciare = trigger_dd["motivo"]
            # Per drawdown -50% forza monetario, per altri cerca sostituto
            if trigger_dd["tipo"] == "drawdown_50":
                t_out = trigger_dd["ticker"]
                etf_out = next((e for e in versione_attiva["composizione"] if e["ticker"] == t_out), None)
                if etf_out:
                    versione_attiva["composizione"] = [e for e in versione_attiva["composizione"] if e["ticker"] != t_out]
                    # Sostituisci con monetario
                    mon = "XEON.MI"
                    sig_mon = etf_data_completo.get(mon, {})
                    p_mon = get_price_on_date(mon, data_corrente, etf_data_completo) or sig_mon.get("price", 1)
                    versione_attiva["composizione"].append({
                        ...
                        "quote": round(etf_out["importo"] / p_mon, 4) if p_mon else 0,
                        "yield_pct": 0,
                    })
        elif cambio_regime:
            deve_ribilanciare = f"cambio_regime {cambio_regime['da']}→{cambio_regime['a']}"

        if deve_ribilanciare and trigger_dd and trigger_dd["tipo"] in ("drawdown_30", "drawdown_20"):
            # Chiudi versione e apri nuova
            cap_new = versione_attiva["capitale_attuale"]
            versione_attiva["data_chiusura"] = data_corrente
            versione_attiva["trigger_chiusura"] = deve_ribilanciare

            ver_num += 1
            po_bt = calc_pesi_override(regime_attivo, 60, {"az_usa": 1.0, "az_europa": 1.0, "az_em": 1.0})
            nuova_comp = seleziona_etf_per_data(
                pool, etf_data_completo, livello_id, alloc_t, n_etf_max,
                data_corrente, po_bt
            )
            if not nuova_comp:
                nuova_comp = seleziona_etf_per_data(pool, etf_data_completo, livello_id, alloc_t, n_etf_max, data_corrente, None)

            # Scala importi al nuovo capitale
            ...
            versione_attiva = {
                "versione": ver_num,
                "livello_id": livello_id,
                "data_apertura": data_corrente,
                "data_chiusura": None,
                "regime": regime_attivo,
                "confidence": cambio_regime["confidence"] if cambio_regime else 60,
                "trigger_apertura": deve_ribilanciare,
                "trigger_chiusura": None,
                ...
                "etf_usciti_anticipati": [],
                "aggiornato": data_corrente,
            }
            versioni.append(versione_attiva)

        elif deve_ribilanciare and cambio_regime:
            # Chiudi versione per cambio regime
            cap_new = versione_attiva["capitale_attuale"]
            versione_attiva["data_chiusura"] = data_corrente
            versione_attiva["trigger_chiusura"] = deve_ribilanciare
            ...
            versione_attiva = {
                ...
                "data_apertura": data_corrente,
                "data_chiusura": None,
                "regime": cambio_regime["a"],
                "confidence": cambio_regime["confidence"],
                "trigger_apertura": deve_ribilanciare,
                "trigger_chiusura": None,
                "capitale_inizio": cap_new,
                "capitale_attuale": cap_new,
    ...
    # Pulisci chiavi interne dagli alert
    for v in versioni:
    ...
    print(f"      → {n_ver} versioni, {n_reb} ribilanciamenti, perf finale {perf_fin:+.1f}%")
    return versioni
...
def main():
    ...
    for ticker in ETF_UNIVERSE:
        ...
        if yahoo_ticker is None:
            ...
            errors += 1
            continue
        print(f"  [{success+errors+1}/{len(ETF_UNIVERSE)}] {ticker}({yahoo_ticker if yahoo_ticker!=ticker else ''})...", end=" ", flush=True)

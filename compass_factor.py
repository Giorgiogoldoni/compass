...
ETF_CATEGORIA = {
    ...
    # Nuovi — Quality e Momentum factor geografici
    "IWQU.MI":"az_globale",  # iShares MSCI World Quality
    "QDVA.DE":"az_usa",      # iShares MSCI USA Momentum
    "QDVB.DE":"az_usa",      # iShares MSCI USA Quality
    "IEMO.MI":"az_europa",   # iShares MSCI Europe Momentum
    "IEQU.MI":"az_europa",   # iShares MSCI Europe Quality
}
...
ETF_TIPO = {
    ...
    # Nuovi — tutti Acc
    "IWQU.MI":"Acc","QDVA.DE":"Acc","QDVB.DE":"Acc","IEMO.MI":"Acc","IEQU.MI":"Acc",
}
...
ETF_NOMI = {
    ...
    # Nuovi — Quality e Momentum factor geografici
    "IWQU.MI":"iShares MSCI World Quality Factor UCITS ETF",
    "QDVA.DE":"iShares MSCI USA Momentum Factor UCITS ETF",
    "QDVB.DE":"iShares MSCI USA Quality Factor UCITS ETF",
    "IEMO.MI":"iShares MSCI Europe Momentum Factor UCITS ETF",
    "IEQU.MI":"iShares MSCI Europe Quality Factor UCITS ETF",
}
...
def calc_rsi_excess_multiplier(rsi14):
    """
    Moltiplicatore adattivo basato sul RSI — penalizza il momentum
    quando il mercato è in eccesso (overbought).

    Zona 1 — Momentum sano    (RSI 45-72): moltiplicatore 1.0 — pieno score
    Zona 2 — Momentum eccesso (RSI 73-82): moltiplicatore 0.7 — penalità moderata
    Zona 3 — Momentum estremo (RSI  > 82): moltiplicatore 0.4 — penalità forte

    Logica: un ETF con RSI 85 e mom3M +60% è in eccesso storico.
    Non lo escludiamo ma riduciamo il suo peso relativo nella selezione,
    favorendo candidati con momentum sano ma meno esteso.
    Questo riduce l'esposizione ai reversal rapidi nelle fasi di mercato
    concentrato — come descritto dall'analisi Investing.com su ampiezza
    in deterioramento e momentum in zona di eccesso.
    """
    if rsi14 is None:
        return 1.0
    if rsi14 > 82:
        return 0.4   # Zona 3 — eccesso estremo
    if rsi14 > 72:
        return 0.7   # Zona 2 — eccesso moderato
    if rsi14 >= 45:
        return 1.0   # Zona 1 — momentum sano
    return 0.8       # RSI < 45 — debolezza, leggera penalità
...
def calc_factor_score(closes, level_id, yield_pct=0):
    ...
    NOVITÀ v1.1 — Score adattivo RSI:
    Il peso del momentum (mom1M, mom3M, mom6M) viene scalato da un
    moltiplicatore RSI che penalizza gli eccessi di mercato.
    Quando RSI > 72 il sistema preferisce candidati con momentum
    sano ma non estremo, riducendo il rischio di reversal.
    """
    ...
    # Moltiplicatore adattivo — scala il momentum in base al RSI
    mom_mult = calc_rsi_excess_multiplier(rsi14)

    s = 0

    # SMA200: prezzo > SMA → pieno punteggio (non scalato — è filtro trend)
    if w["sma200"] > 0 and sma200:
        s += w["sma200"] if price > sma200 else 0

    # KAMA: prezzo > KAMA → pieno punteggio (non scalato — è filtro trend)
    if w["kama"] > 0 and kama:
        s += w["kama"] if price > kama else 0

    # Mom 1M: scala lineare, max a +20% — SCALATO da mom_mult
    if w["mom1m"] > 0 and mom1m is not None:
        w_mom1m = w["mom1m"] * mom_mult  # peso effettivo ridotto in eccesso
        if mom1m >= 0:
            s += round(min(w_mom1m, (mom1m/20)*w_mom1m))
        else:
            s += round(max(-w_mom1m/2, (mom1m/20)*w_mom1m))

    # Mom 3M: scala lineare, max a +30% — SCALATO da mom_mult — cuore dello score
    if w["mom3m"] > 0 and mom3m is not None:
        w_mom3m = w["mom3m"] * mom_mult  # peso effettivo ridotto in eccesso
        if mom3m >= 0:
            s += round(min(w_mom3m, (mom3m/30)*w_mom3m))
        else:
            s += round(max(-w_mom3m/2, (mom3m/30)*w_mom3m))

    # Mom 6M: scala lineare, max a +50% — SCALATO da mom_mult
    if w["mom6m"] > 0 and mom6m is not None:
        w_mom6m = w["mom6m"] * mom_mult  # peso effettivo ridotto in eccesso
        if mom6m >= 0:
            s += round(min(w_mom6m, (mom6m/50)*w_mom6m))
        else:
            s += round(max(-w_mom6m/2, (mom6m/50)*w_mom6m))

    # RSI: zone di qualità del momentum (non scalato — è già nel moltiplicatore)
    # Zona sana 55-72: pieno · Zona eccesso 73-82: ridotto · Estremo >82: minimo
    if w["rsi"] > 0 and rsi14 is not None:
        if 55 <= rsi14 <= 72:
            s += w["rsi"]           # Zona 1 — momentum sano: pieno
        elif 45 <= rsi14 < 55:
            s += round(w["rsi"] * 0.5)  # Zona di transizione
        elif 73 <= rsi14 <= 82:
            s += round(w["rsi"] * 0.3)  # Zona 2 — eccesso moderato
        elif rsi14 > 82:
            s += 0                  # Zona 3 — eccesso estremo: zero
        # RSI < 45: 0 punti — debolezza

    # ADX: forza del trend — ottimale > 25 (non scalato)
    ...
    # AO: Awesome Oscillator positivo = momentum confermato (non scalato)

...
# ── BACKTEST LINEA O — SEPARATO E CONTROLLATO ─────────────────────────────
def run_backtest_O(etf_data, proxy_data):
    """
    Backtest dedicato linea O v2.0.
    Logica completamente separata da M e A:
    - Composizione statica per regime (COMPOSIZIONE_O_PER_REGIME)
    - Cambio composizione solo su cambio regime conf>=65% e persistenza>=3 settimane
    - Floor mensile -3%: se superato -> sposta su IBGS.MI (gov breve difensivo)
    - Nessuna rotation momentum
    - Cedola inclusa nel total return
    """
    print(f"    Backtest linea O (Obbligazionario v2.0 — statico per regime)...")
    ...
    def get_price(ticker, date):
        ...
        dates  = sig.get("dates", [])
        best = None
        for i in range(min(len(closes), len(dates))):
            if dates[i] <= date:
                best = closes[i]
            elif dates[i] > date:
                break
    ...
    def build_comp(regime, capital, date):
        """Costruisce composizione dal template regime con prezzi reali."""
        template = COMPOSIZIONE_O_PER_REGIME.get(regime, COMPOSIZIONE_O_PER_REGIME["neutro"])
        comp = []
        tot_peso = 0
        for item in template:
            t = item["ticker"]
            ...
            if not price:
                print(f"      ⚠ {t} prezzo non disponibile al {date}")
                continue
            peso = item["peso"]
            importo = round(capital * peso / 100, 2)
            comp.append({
                "ticker": t,
                "nome": ETF_NOMI.get(t, t),
                "peso": float(peso),
                "importo": importo,
                "price_entry": round(price, 4),
                "quote": round(importo / price, 4),
                "yield_pct": YIELD_STIMATI_O.get(t, 0),
                "score_entry": 0,
                "mom3m_entry": 0.0,
            })
            tot_peso += peso
        # Se mancano ETF, redistribuisci su quelli disponibili
        if comp and tot_peso < 95:
            print(f"      ⚠ Solo {tot_peso}% allocato — redistribuisco")
            factor = 100.0 / tot_peso
            for e in comp:
                e["peso"] = round(e["peso"] * factor, 1)
                e["importo"] = round(capital * e["peso"] / 100, 2)
                e["quote"] = round(e["importo"] / e["price_entry"], 4) if e["price_entry"] else 0
        return comp

    # Calcola cedola aggregata
    def calc_cedola_agg(comp):
        return round(sum(e["yield_pct"] * e["peso"] / 100 for e in comp), 2)

    # Inizializzazione V1 — regime neutro al 2025-01-01
    regime_attivo = "neutro"
    conf_attiva = 50
    cap = float(CAPITALE)
    comp_v1 = build_comp("neutro", cap, BACKTEST_START)
    if not comp_v1:
        print("      ❌ Impossibile costruire composizione V1")
        return None
    ...
    ver_attiva = {
        "versione": 1, "linea_id": "O",
        "data_apertura": BACKTEST_START, "data_chiusura": None,
        "regime": "neutro", "confidence": 50,
        "trigger_apertura": "inizializzazione", "trigger_chiusura": None,
        ...
        "_equity": [cap],
        "cedola_aggregata_pct": calc_cedola_agg(comp_v1),
    }
    ...
    # Tracking floor mensile
    cap_inizio_mese = cap
    mese_corrente = BACKTEST_START[:7]
    ...
    for data_corrente in all_dates:
        ...
        # Calcola valore corrente portafoglio
        tot_v = 0.0
        for etf in ver_attiva["composizione"]:
            t = etf["ticker"]
            if etf.get("importo", 0) <= 0:
                continue
            price_now = get_price(t, data_corrente)
            if not price_now:
        ...
        cap_now = round(tot_v, 2)
        ver_attiva["capitale_attuale"] = cap_now
        ver_attiva["performance_eur"] = round(cap_now - ver_attiva["capitale_inizio"], 2)
        ver_attiva["performance_pct"] = round(
            (cap_now - ver_attiva["capitale_inizio"]) / ver_attiva["capitale_inizio"] * 100, 2)
        ver_attiva["aggiornato"] = data_corrente
        ver_attiva.setdefault("_equity", []).append(cap_now)
        equity_curve.append(cap_now)
        equity_dates.append(data_corrente)

        # Rendimenti mensili
        mese = data_corrente[:7]
        if mese != mese_corrente:
            # Chiudi mese precedente
            if cap_inizio_mese > 0:
                rend_mese = round((cap_now - cap_inizio_mese) / cap_inizio_mese * 100, 2)
                rendimenti_mensili[mese_corrente] = rend_mese
            cap_inizio_mese = cap_now
            mese_corrente = mese
        ...
        if giorni_check >= 5:
            ...
            # Floor mensile: se perdita dal cap inizio versione >= 3%
            perf_ver = (cap_now - ver_attiva["capitale_inizio"]) / ver_attiva["capitale_inizio"] * 100
            if perf_ver <= O_FLOOR_MENSILE and giorni >= 20:
                floor_breach = True

            # Cambio regime robusto: conf>=65%, persistenza>=3 settimane
            if sc != regime_attivo and not floor_breach:
                if sc == regime_cand:
                    settimane_cand += 1
                else:
                    regime_cand = sc
                    settimane_cand = 1
                if (settimane_cand >= O_PERSIST_SETTIMANE and
                        conf >= O_CONF_MIN_REGIME and giorni >= 30):
                    cambio_regime = {"da": regime_attivo, "a": sc, "confidence": conf}
                    regime_attivo = sc
                    conf_attiva = conf
                    settimane_cand = 0
                    regime_cand = None
            elif sc == regime_attivo:
                settimane_cand = 0
                regime_cand = None

        # ── RIBILANCIAMENTO ─────────────────────────────────────────────────
        if cambio_regime or floor_breach:
            trigger_str = (f"floor_mensile {perf_ver:.1f}% <= {O_FLOOR_MENSILE}%" 
                          if floor_breach 
                          else f"cambio_regime {cambio_regime['da']}→{cambio_regime['a']} (conf {cambio_regime['confidence']}%)")
            nuovo_regime = "risk_off" if floor_breach else cambio_regime["a"]
            nuova_conf = 99 if floor_breach else cambio_regime["confidence"]

            # Salva composizione venduta
            venduti = [{
                "ticker": e["ticker"], "nome": e.get("nome", e["ticker"]),
                "peso": e["peso"], "importo": e["importo"],
                "price_entry": e["price_entry"],
                "price_exit": e.get("price_now", e["price_entry"]),
                "perf_pct": e.get("perf_pct", 0),
                "perf_eur": e.get("perf_eur", 0),
            } for e in ver_attiva["composizione"] if e.get("importo", 0) > 0]

            # Nuova composizione
            nuova_comp = build_comp(nuovo_regime, cap_now, data_corrente)
            if not nuova_comp:
                continue  # non ribilanciare se prezzi non disponibili
            ...
            acquistati = [{
                ...
                "peso": e["peso"], "importo": e["importo"],
                "price_entry": e["price_entry"],
                "score_entry": 0, "mom3m_entry": 0.0,
            } for e in nuova_comp]

            ver_attiva["data_chiusura"] = data_corrente
            ver_attiva["trigger_chiusura"] = trigger_str
            ver_num += 1

            ver_attiva = {
                "versione": ver_num, "linea_id": "O",
                "data_apertura": data_corrente, "data_chiusura": None,
                "regime": nuovo_regime, "confidence": nuova_conf,
                "trigger_apertura": trigger_str, "trigger_chiusura": None,
                "capitale_inizio": cap_now, "capitale_attuale": cap_now,
                "performance_pct": 0.0, "performance_eur": 0.0,
                "giorni_attivo": 0, "composizione": nuova_comp,
                "etf_usciti": [], "aggiornato": data_corrente,
                "_equity": [cap_now],
                "cedola_aggregata_pct": calc_cedola_agg(nuova_comp),
            }
            versioni.append(ver_attiva)

            # Aggiunge al diario (costruito dopo)
            ver_attiva["_venduti"] = venduti
            ver_attiva["_acquistati"] = acquistati

    # Chiudi ultimo mese
    if mese_corrente and cap_inizio_mese > 0:
        rend_mese = round((equity_curve[-1] - cap_inizio_mese) / cap_inizio_mese * 100, 2)
        rendimenti_mensili[mese_corrente] = rend_mese
    ...
    for v in versioni[1:]:
        diario.append({
            "data": v["data_apertura"],
            "versione_da": v["versione"] - 1,
            "versione_a": v["versione"],
            "trigger": v["trigger_apertura"],
            "regime": v["regime"],
            "capitale_ribilanciamento": v["capitale_inizio"],
            "venduti": v.pop("_venduti", []),
            "acquistati": v.pop("_acquistati", []),
        })
    ...
    cedola_fin = ver_attiva.get("cedola_aggregata_pct", 0)
    cedola_ok = cedola_fin >= O_CEDOLA_MIN_NETTA

    vol_str = f", vol {vol_tot:.1f}%, MDD {mdd_tot:.1f}%, Sharpe {sharpe_tot:.2f}" if vol_tot else ""
    cedola_str = f", cedola {cedola_fin:.1f}% {'✅' if cedola_ok else '❌'}"
    print(f"      → {len(versioni)} ver, {len(diario)} rib, perf {perf_fin:+.1f}%{vol_str}{cedola_str}")
    ...
    return {
        ...
        "sharpe": sharpe_tot,
        "rendimenti_mensili": rendimenti_mensili,
        "diario_movimenti": diario,
        "versione_corrente": len(versioni),
        "benchmark": LINEE_INFO["O"]["benchmark"],
        "rotation_suggerita": None,
        "cedola_aggregata_pct": cedola_fin,
        "cedola_ok": cedola_ok,
        "outperformance_bm": None,
        "batte_benchmark": None,
    }

# ── BACKTEST LINEA M e A ───────────────────────────────────────────────────
def run_backtest_linea(linea_id, etf_data, proxy_data):
    ...
    # Linea M: statica, nessun cambio regime mai
    # Linea O: gestita da run_backtest_O, non arriva qui
    # Linea A: usa PERSISTENZA standard
    pers = PERSISTENZA
    ...
    for data_corrente in all_dates:
        ...
        if giorni_check >= 5:
            ...
            # Linea M: nessun cambio regime, nessuna rotation — statica assoluta
            if linea_id != "M":
                sc, conf = classify_regime_at_date(proxy_data, data_corrente)
                if sc != regime_attivo:
                    if sc == regime_cand:
...
def main():
    ...
    if OUT_FILE.exists():
        try:
            ...
            # Se versione precedente, forza reset completo
            if existing.get("version","") != "3linee_2.0":
                print(f"  Versione precedente ({existing.get('version','?')}) — forza backtest completo")
                existing = {}
            else:
    ...
    # Versione O cambiata — forza backtest da zero
    FORCE_RESET = {"O"}  # Reset linee con logica cambiata

    for linea_id in LINEE:
        if linea_id in portafogli_esistenti and linea_id not in FORCE_RESET:
        ...
        # Backtest da zero — linea O usa funzione dedicata
        if linea_id == "O":
            risultato = run_backtest_O(etf_data, proxy_data)
        else:

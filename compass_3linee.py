...
LINEE_INFO = {
    "M": {
        "nome": "Monetario",
        "obiettivo": "Protezione capitale e ottimizzazione yield",
        "benchmark": "XEON.MI",
        "n_etf_max": 3,
    ...
    "O": {
        ...
        "obiettivo": "Rendimento obbligazionario con rotation momentum e regime",
        "benchmark": "IEAG.MI",
        "n_etf_max": 6,
        "pool": [
            # IG
            "IEAC.MI", "IEAG.MI", "JNHD.DE", "XUCS.DE",
            # Duration
            "IBGM.MI", "EUNH.DE", "SEGA.MI",
            # HY
            "IHYU.MI", "EUHI.MI", "HYLD.MI", "STHE.MI", "STHY.MI",
            # EM Bond
            "EMBE.MI", "EMDV.MI",
            # Global hedged
            "XGSH.MI",
    ...
    "A": {
        "nome": "Azionario",
        "obiettivo": "Crescita alta composta — logica A7 Factor",
        "benchmark": "IWMO.MI",
        "n_etf_max": 8,
        "pool": [
            # Obbligazionario residuale difensivo
            "IEAC.MI", "VAGE.DE", "EMBE.MI",
            # HY difensivo
            "EUHA.MI", "HYLD.MI", "JNHD.DE",
            # Azionario globale
...
# Per ora usato solo come riferimento documentale
RANGE_REGIME = {
...
ETF_CATEGORIA = {
    ...
    # HY
    "IHYU.MI":"hy","EUHI.MI":"hy","HYLD.MI":"hy","STHE.MI":"hy","STHY.MI":"hy","EUHA.MI":"hy",
    # EM Bond
    ...
    "VAGE.DE":"obbligaz_ig","EMBE.MI":"em_bond","EUHA.MI":"hy","JNHD.DE":"obbligaz_ig",
}

ETF_NOMI = {
    "XEON.MI":"Amundi EUR Overnight Return","SMART.MI":"iShares EUR Ultrashort Bond",
    ...
    "VAGE.DE":"Vanguard EUR Aggregate Bond","EUHA.MI":"iShares EUR High Yield Corp Bond",
...
ETF_TIPO = {
    ...
    "VAGE.DE":"Dist","EUHA.MI":"Dist","JNHD.DE":"Dist","XUTC.MI":"Dist",
}
...
# ── SELEZIONE ETF ──────────────────────────────────────────────────────────
def seleziona_etf(pool, linea_id, etf_data, n_max, target_date, regime=None):
    """
    Selezione ETF per una linea in una data storica.
    Linea M: ordina per score yield-stabilità
    Linea O/A: ordina per score momentum adattivo
    """
    candidati = []
    for t in pool:
        sig = etf_data.get(t)
        ...
        # Linea O: penalizza HY in regime risk_off/stagflazione
        if linea_id == "O" and regime in ("risk_off", "stagflazione"):
            cat = ETF_CATEGORIA.get(t, "")
            if cat == "hy":
                continue  # escludi HY in regime difensivo
...
def run_backtest_linea(linea_id, etf_data, proxy_data):
    ...
    comp_v1 = seleziona_etf(pool, linea_id, etf_data, n_max, BACKTEST_START, "neutro")
    ...
    def apri_nuova(cap_new, data, regime, conf, trigger, reg=None):
        ...
        ver_num += 1
        nuova_comp = seleziona_etf(pool, linea_id, etf_data, n_max, data, reg)
        if not nuova_comp: return
        scala = cap_new / CAPITALE if CAPITALE else 1
        for e in nuova_comp:
    ...
    for data_corrente in all_dates:
        ...
        for etf in ver_attiva["composizione"]:
            ...
            dd_soglia = {"M": -10, "O": -15, "A": -25}.get(linea_id, -20)
        ...
        if giorni_check >= 5:
            ...
            # Rotation (non per linea M)
            if not trigger_dd and not cambio_regime and linea_id != "M":

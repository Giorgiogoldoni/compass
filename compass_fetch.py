...
def classify_regime(proxy_data):
    ...
    # Segnale geografico: STOXX/SPY e EEM/SPY
    geo_signal = {"az_usa": 1.0, "az_europa": 1.0, "az_em": 1.0}
    vgk = get_ret4w("VGK"); spy_r = get_ret4w("SPY"); eem_r = get_ret4w("EEM")
    if vgk is not None and spy_r is not None and spy_r != 0:
        ratio_eu = vgk / spy_r if spy_r else 1.0
        geo_signal["az_europa"] = round(max(0.3, min(1.8, ratio_eu)), 2)
        geo_signal["az_usa"]    = round(max(0.3, min(1.8, 2.0 - ratio_eu)), 2)
    if eem_r is not None and spy_r is not None and spy_r != 0:
        ratio_em = eem_r / spy_r if spy_r else 1.0
        geo_signal["az_em"] = round(max(0.3, min(1.8, ratio_em)), 2)
...
def gestisci_portafogli(portafogli_esistenti, etf_data, regime_corrente,
    ...
    for lv in levels_data:
        ...
        for trig in triggers_dd:
            ...
            if (trig["tipo"] in ("drawdown_30", "drawdown_50") or
                ...
                storia.append({
                    ...
                    "trigger_chiusura": None,
                    "capitale_inizio": ver_attiva.get("capitale_attuale", CAPITALE_MODELLO),
                    "capitale_attuale": ver_attiva.get("capitale_attuale", CAPITALE_MODELLO),
                    "performance_pct": 0.0,
                    "performance_eur": 0.0,
                    "giorni_attivo": 0,
        ...
        # Performance totale cumulativa
        perf_tot = sum(v.get("performance_eur", 0) for v in storia)
        portafogli[lid]["performance_totale_eur"] = round(perf_tot, 2)
        portafogli[lid]["performance_totale_pct"] = round(perf_tot / CAPITALE_MODELLO * 100, 2)

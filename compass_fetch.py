...
def run_backtest(livello_id, etf_data_completo, alloc, n_etf_max,
    ...
    def get_regime_for_date(d):
        ...
        if d < "2025-09-01": return ("goldilocks", 62)
        if d < "2025-12-01": return ("neutro", 58)
        if d < "2026-02-01": return ("goldilocks", 60)
        if d < "2026-04-01": return ("neutro", 57)
        if d < "2026-05-01": return ("risk_off", 72)
    ...
    for data_corrente in all_dates:
        ...
        if giorni_da_check >= 5 and versione_attiva["giorni_attivo"] >= 45:
            ...
            for etf in versione_attiva["composizione"]:
                ...
                t = etf["ticker"]
                sig = etf_data_completo.get(t, {})
                score_now = sig.get("score_by_level", {}).get(livello_id, sig.get("score", 50))
                if score_now is not None and score_now < 15:
                    score_basso_count[t] = score_basso_count.get(t, 0) + 1
                    if score_basso_count[t] >= 3:  # 3 settimane consecutive sotto 15

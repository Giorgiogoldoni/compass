...
BENCHMARK_TICKERS = ["MACV.MI", "MODR.MI", "MAGR.MI", "SWDA.MI", "IEAG.MI", "IWMO.MI"]
BENCHMARK_NOMI = {
...
def calc_signal_history(closes, kama_series, sar_series):
    """
    Genera storia segnali BUY/SELL basata su:
    - KAMA crossover (prezzo incrocia KAMA)
    - SAR inversione
    Restituisce lista di {idx, tipo, prezzo}
    """
    signals = []
    ...
    for i in range(1, n):
        ...
        if prev_above_kama is not None:
            if above_kama and not prev_above_kama:
                signals.append({"idx": i, "tipo": "BUY", "prezzo": round(p,4)})
            elif not above_kama and prev_above_kama:
                signals.append({"idx": i, "tipo": "SELL", "prezzo": round(p,4)})
...
def main():
    ...
    for ticker in universo_run:
        ...
        signals  = calc_signal_history(closes_252, kama_s, sar_s)

...
Campi aggiuntivi per ETF a distribuzione:
  yield_annuo      — % rendimento cedolare su prezzo attuale (ultimi 12m)
  dividendi_12m    — € totale dividendi pagati negli ultimi 12 mesi
  frequenza        — "mensile" / "trimestrale" / "semestrale" / "annuale"
  tipo             — "Dist" / "Acc"
  ex_div_date      — data ultimo stacco cedola (YYYY-MM-DD)
"""
...
# ── YIELD & DISTRIBUZIONE ────────────────────────────────────────
def get_distribuzione(ticker: str, price: float):
    """
    Scarica dividendi da yfinance e calcola:
    - tipo: Dist / Acc
    - yield_annuo: % cedola su prezzo (ultimi 12 mesi)
    - dividendi_12m: EUR totale pagati negli ultimi 12m
    - frequenza: mensile / trimestrale / semestrale / annuale
    - ex_div_date: data ultimo stacco
    """
    result = {
        "tipo": "Acc",
        "yield_annuo": None,
    ...
    try:
        ...
        info = tkr.info or {}
        trailing_yield = info.get("dividendYield")

        # Dividendi storici ultimi 12 mesi
        divs = tkr.dividends
        if divs is not None and len(divs) > 0:
            cutoff = pd.Timestamp.now(tz="UTC") - pd.Timedelta(days=365)
            divs_12m = divs[divs.index >= cutoff]
            total_12m = float(divs_12m.sum()) if len(divs_12m) > 0 else 0.0

            if total_12m > 0 and price > 0:
                result["tipo"] = "Dist"
                result["dividendi_12m"] = round(total_12m, 4)
                result["yield_annuo"]   = round((total_12m / price) * 100, 2)

                # Frequenza: conta pagamenti negli ultimi 12m
                n = len(divs_12m)
                if   n >= 11: result["frequenza"] = "mensile"
                elif n >= 3:  result["frequenza"] = "trimestrale"
                elif n >= 2:  result["frequenza"] = "semestrale"
                elif n == 1:  result["frequenza"] = "annuale"

                # Ultimo ex-div date
                last_date = divs_12m.index[-1]
                result["ex_div_date"] = str(last_date.date())
            else:
                result["tipo"] = "Acc"

        # Fallback: usa Yahoo dividendYield
        elif trailing_yield and trailing_yield > 0 and price > 0:
            result["tipo"] = "Dist"
            result["yield_annuo"] = round(trailing_yield * 100, 2)
            result["dividendi_12m"] = round(trailing_yield * price, 4)
...
def main():
    ...
    for ticker in ETF_UNIVERSE:
        try:
            ...
            # Yield e distribuzione
            dist = get_distribuzione(ticker, price)
            ...
            results[ticker] = {
                ...
                "tipo":          dist["tipo"],
                "yield_annuo":   dist["yield_annuo"],
                "dividendi_12m": dist["dividendi_12m"],
                "frequenza":     dist["frequenza"],
                "ex_div_date":   dist["ex_div_date"],
                # Prezzi per sparkline
                "prices_60":     [round(float(v), 4) for v in close.tail(60).tolist()],
            }
            ok_count += 1
            yield_str = f" yield={dist['yield_annuo']}% {dist['frequenza'] or ''}" if dist["tipo"] == "Dist" else " Acc"
            print(f"  ✓ {ticker}: {segnale} ({score}/4) prezzo={price:.2f}{yield_str}")
